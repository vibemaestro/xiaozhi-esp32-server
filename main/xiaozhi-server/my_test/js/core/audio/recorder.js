...
export class AudioRecorder {
        ...
        return `
            class AudioRecorderProcessor extends AudioWorkletProcessor {
                constructor() {
                    ...
                    this.port.onmessage = (event) => {
                        if (event.data.command === 'start') {
                            ...
                            this.isContinuousMode = event.data.continuous || false;
                            ...
                            this.isRecording = true;
                            this.port.postMessage({ type: 'status', status: 'resumed' });
                ...
                process(inputs, outputs, parameters) {
                    if (!this.isRecording && !this.isContinuousMode) return true;
    ...
    processPCMBuffer(buffer) {
        ...
        while (this.pcmDataBuffer.length >= samplesPerFrame) {
            ...
            if (this.isContinuousMode && this.isStreaming) {
                this.detectAndHandleVAD(frameData);
            }
    ...
    detectAndHandleVAD(buffer) {
        ...
        if (rms < this.silenceThreshold) {
            ...
            if (!this.isStreaming && this.onSpeechStart) {
                this.onSpeechStart();
    ...
    async start() {
        ...
        try {
            ...
            if (this.audioProcessorType === 'worklet' && this.audioProcessor.port) {
                this.audioProcessor.port.postMessage({ command: 'start', continuous: false });
            }
    ...
    // Start continuous recording (realtime VAD mode)
    async startContinuous() {
        if (this.isContinuousMode) return false;
        try {
            if (!this.initEncoder()) {
                log('Cannot start continuous recording: Opus encoder initialization failed', 'error');
                return false;
            }
            if (!this.stream) {
                const stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, sampleRate: 16000, channelCount: 1 } });
            ...
            this.silenceFrameCount = 0;
            if (this.audioProcessorType === 'worklet' && this.audioProcessor.port) {
                this.audioProcessor.port.postMessage({ command: 'start', continuous: true });
            }
            if (this.onVisualizerUpdate) {
                const dataArray = new Uint8Array(this.analyser.frequencyBinCount);
                this.startVisualization(dataArray);
            ...
            log('Started continuous realtime recording', 'success');
            return true;
        } catch (error) {
            log(`Continuous recording start error: ${error.message}`, 'error');
            this.isContinuousMode = false;
            this.isRecording = false;
            return false;
        }
    ...
    // Pause recording (for continuous mode)
    pause() {
        if (!this.isContinuousMode || !this.isRecording) return false;
        try {
            this.isRecording = false;
            if (this.audioProcessorType === 'worklet' && this.audioProcessor.port) {
                this.audioProcessor.port.postMessage({ command: 'pause' });
            }
            log('Paused continuous recording', 'info');
            return true;
        } catch (error) {
            log(`Pause recording error: ${error.message}`, 'error');
            return false;
        }
    }

    // Resume recording (for continuous mode)
    resume() {
        if (!this.isContinuousMode || this.isRecording) return false;
        try {
            this.isRecording = true;
            if (this.audioProcessorType === 'worklet' && this.audioProcessor.port) {
                this.audioProcessor.port.postMessage({ command: 'resume' });
            }
            log('Resumed continuous recording', 'info');
            return true;
        } catch (error) {
            log(`Resume recording error: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    // Stop recording
    stop() {
        if (!this.isRecording && !this.isContinuousMode) return false;
        try {
            this.isRecording = false;
            this.isContinuousMode = false;
            this.isStreaming = false;
