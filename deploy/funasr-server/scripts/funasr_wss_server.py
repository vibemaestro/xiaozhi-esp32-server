...
parser.add_argument(
    "--asr_model_remote_code",
    type=str,
    default="./model.py",
    help="remote code path",
)
...
model_asr = AutoModel(
    ...
    ncpu=args.ncpu,
    device=args.device,
    remote_code=args.asr_model_remote_code,
...
async def ws_reset(websocket):
    ...
    if getattr(websocket, "status_dict_vad", None) is not None:
...
async def ws_serve(websocket, path=None):
    ...
    websocket.is_speaking = True  # default until client sends flag
    websocket.status_dict_asr = {}
    websocket.status_dict_asr_online = {"cache": {}, "is_final": False, "chunk_size": [0, 10, 5]}
    websocket.status_dict_vad = {"cache": {}, "is_final": False} if model_vad is not None else None
    websocket.status_dict_punc = {"cache": {}}
            ...
            if len(frames_asr_online) > 0 or len(frames_asr) >= 0 or not isinstance(message, str):
                if not isinstance(message, str):
                    ...
                    if (
                        ...
                        if websocket.mode == "2pass" or websocket.mode == "online":
                            ...
                            try:
                                ...
                                logging.exception(
                                    "error in asr streaming with status %s", websocket.status_dict_asr_online
                                )
                        frames_asr_online = []
                    if speech_start:
                    ...
                    if model_vad is None:
                        # Without VAD, process continuously
                        speech_start = True
                    elif speech_start_i != -1:
                        speech_start = True
                ...
                if speech_end_i != -1 or not websocket.is_speaking:
                    ...
                    if websocket.mode == "2pass" or websocket.mode == "offline":
                        ...
                        try:
                            ...
                            logging.exception("error in asr offline")
                    frames_asr = []
                    speech_start = False
                    frames_asr_online = []
                    websocket.status_dict_asr_online["cache"] = {}
                    if not websocket.is_speaking:
                        websocket.vad_pre_idx = 0
                        frames = []
                        if websocket.status_dict_vad is not None:
                            websocket.status_dict_vad["cache"] = {}
                    else:
                        frames = frames[-20:]
...
async def async_asr(websocket, audio_in):
    if len(audio_in) > 0:
        ...
        try:
            ...
            logging.exception(
                "offline inference IndexError, resetting cache; status=%s", websocket.status_dict_asr
            )
...
async def async_asr_online(websocket, audio_in):
    if len(audio_in) > 0:
        ...
        try:
            ...
            # Upstream model threw because of empty/invalid chunk, reset cache and continue
            logging.exception(
                "online inference IndexError, resetting cache; status=%s", websocket.status_dict_asr_online
            )
            websocket.status_dict_asr_online["cache"] = {}
            return
        except Exception:
            # Bubble other errors up (already caught by caller)
            raise
