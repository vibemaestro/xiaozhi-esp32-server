...
class Funasr_websocket_recognizer:
    ...
    def __init__(
        ...
        try:
            ...
            # Server expects subprotocol "binary"
            self.websocket = create_connection(
                uri, ssl=ssl_context, sslopt=ssl_opt, subprotocols=["binary"]
            )
...
if __name__ == "__main__":
    ...
    wav_path = "/workspace/test/samples/sample_vi.wav"
    ...
    rcg = Funasr_websocket_recognizer(
        host="127.0.0.1", port="10095", is_ssl=False, mode="2pass", chunk_size="0,10,5"
    )
    # loop to send chunk
